...
int jo_epoll_create(int flags){//如果epoll_create1成功，则本函数为events分配了最大大小
...
int jo_epoll_mod(int epoll_fd,int fd,jo_http_request_t request,int events){//修改描述符状态
    struct epoll_event event;
    event.data.ptr = (void*)request;
    event.events = events;
...
int jo_handler(int epoll_fd,int listen_fd,struct epoll_event * events,
    ...
    for(int i=0;i<events_num;i++){
        jo_http_request* request=(jo_http_request*)(events[i].data.ptr);//读取事件里的地址
